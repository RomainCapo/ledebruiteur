...
def wiener_filter(img, unsupervised=True, wiener_balance = 1100, psf_size=5, psf_numerator=25):
    ...
    Keyword Arguments:
        unsupervised {bool} -- true for supervised algorithme, false otherwise (default: {True})
    ...
    convolved_img += 0.1 * convolved_img.std() * np.random.standard_normal(convolved_img.shape)
...
def conservative_filter(img, filter_size):
    ...
    Code become from : https://towardsdatascience.com/image-filters-in-python-26ee938e57d2
